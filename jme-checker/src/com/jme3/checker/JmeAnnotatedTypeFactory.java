...
/**
 * Adds implicit and default Jme annotations, only if the user does not
 ...
 * </ul>
 */
public class JmeAnnotatedTypeFactory extends AnnotatedTypeFactory {

    /** Adds annotations from tree context before type resolution. */
    private final JmeTreePreAnnotator treePre;

    /** Adds annotations from the resulting type after type resolution. */
    private final JmeTypePostAnnotator typePost;

    /** The Jme annotations. */
    private final AnnotationMirror READONLY, DESTRUCTIVE;

    /**
     * Creates a new {@link JmeAnnotatedTypeFactory} that operates on a
     * particular AST.
     *
     * @param checker the checker to which this factory belongs
     * @param root the AST on which this type factory operates
     */
    public JmeAnnotatedTypeFactory(JmeChecker checker,
        CompilationUnitTree root) {
        super(checker, root);
        this.treePre = new JmeTreePreAnnotator();
        this.typePost = new JmeTypePostAnnotator();
        this.READONLY = checker.READONLY;
        this.DESTRUCTIVE = checker.DESTRUCTIVE;
        postInit();
    }
...
//        }
        ...
        if (type.hasAnnotation(READONLY)) {
            ...
            if (!exprReceiver.hasAnnotation(READONLY)) {
                exprReceiver.addAnnotation(READONLY);
            }
        }
...
//            new AnnotatedTypeReplacer(POLYREAD, replacement).visit(type);
     ...
     *
     * <pre>@PolyRead Object breakJme(@PolyRead Object s) @ReadOnly {
     *   tmObject = s;
     *   return null;
     *  }
     * </pre>
...
//        }
     ...
     */
    private class JmeTreePreAnnotator extends SimpleTreeVisitor<Void, AnnotatedTypeMirror> {
...
//                p.addAnnotation(MUTABLE);
     ...
     * </ul>
     */
    private class JmeTypePostAnnotator extends AnnotatedTypeScanner<Void, ElementKind> {
