...
public class LzmaReadableChannel implements ReadableByteChannel {

    private ReadableByteChannel in;

    private boolean isClosed;
    private CRangeDecoderChannel RangeDecoder;
    private byte  [] dictionary;
    private int dictionarySize;
    private int dictionaryPos;
    private int GlobalPos;
    private int rep0, rep1, rep2, rep3;
    private int lc;
    private int lp;
    private int pb;
    private int State;
    ...
    public LzmaReadableChannel(ReadableByteChannel in) throws IOException {
        this.in = in;
        isClosed = false;
        readHeader();
        fill_buffer();
	...
	private void readHeader() throws IOException {
		byte [] properties = new byte[5];
                ByteBuffer bb = ByteBuffer.wrap(properties);
                bb.clear();
		if (in.read(bb) != 5)
		...
		RangeDecoder = new CRangeDecoderChannel(in);
		dictionaryPos = 0;
		GlobalPos = 0;
    ...
    public int read(ByteBuffer buf) throws IOException{
        ...
        int l = Math.min(buf.remaining(), uncompressed_size-uncompressed_offset);
        buf.put(uncompressed_buffer, uncompressed_offset, l);
        uncompressed_offset += l;
        return l;
    }

    public boolean isOpen(){
        return !isClosed && in.isOpen();
    }
