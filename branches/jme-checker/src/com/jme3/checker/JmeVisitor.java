...
/**
 * A type-checking visitor for the Jme mutability annotations
 * ({@code @ReadOnly}, {@code @Mutable} and {@code @Assignable}) that
 * extends BaseTypeVisitor.
 *
 * @see BaseTypeVisitor
 */
public class JmeVisitor extends BaseTypeVisitor<Void, Void> {

    final private AnnotationMirror READONLY, DESTRUCTIVE;

    /**
     * Creates a new visitor for type-checking the Jme mutability
     * annotations.
     *
     * @param checker the {@link JmeChecker} to use
     * @param root the root of the input program's AST to check
     */
    public JmeVisitor(JmeChecker checker, CompilationUnitTree root) {
        super(checker, root);
        READONLY = checker.READONLY;
        DESTRUCTIVE = checker.DESTRUCTIVE;
...
//                checker.report(Result.failure("ro.field"), varTree);
    ...
    public void validateTypeOf(Tree tree) {
        ...
        if (type.getKind().isPrimitive()) {
            if (type.hasAnnotation(DESTRUCTIVE)
