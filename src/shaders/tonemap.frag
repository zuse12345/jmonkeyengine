...
void main() {
    vec2 avgLumA = texture2D(m_Lum, vec2(0.0)).rg;
    vec2 avgLumB = texture2D(m_Lum2, vec2(0.0)).rg;
    vec2 lerpedLum = mix(avgLumA, avgLumB, m_BlendFactor);

    vec4 color = texture2D(m_Texture, texCoord);
    vec3 c1 = HDR_ToneMap(color.rgb, lerpedLum.x, m_A, m_White);
